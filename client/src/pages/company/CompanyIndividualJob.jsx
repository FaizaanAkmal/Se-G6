...
const JobPostPage = () => {
  ...
  const applicants = [
    { name: 'John Doe', qualification: 'Passed', experience: '5+ Years', skills: '5/5', status: 'undecided' },
    { name: 'Haskell', qualification: 'Failed', experience: '2+ Years', skills: '2/5', status: 'rejected'   },
    { name: 'Guy', qualification: 'Passed', experience: '10+ Years', skills: '5/5', status: 'shortlisted'},
  ];
  ...
  const handleTabChange = (index) => {
    ...
    return (
          ...
          <Grid container justifyContent="center">
              <Grid item xs={12} md={8} lg={6}>
                  {loading ? (
                    ...
                    <Stack spacing={3} border={1}  borderRadius={4} borderColor="#E1E5EA" padding={3}  >
                        {/* Rest of your component */}
                        {/* Job Title */}
                        <Stack spacing={2}>
                          <Typography textAlign={"left"} variant="h1" fontSize={30} color="#101828" fontWeight={550}>
                          ...
                          <Stack direction="row" spacing={1} borderLeft={8} borderColor="#FFFFFF">
                              <CalendarToday />
                              <Typography variant="body2" fontSize={12} textColor={475467}> {formatDate(applicants_real.datePosted)}</Typography>
                              {applicants_real.status === 'Open' ? <Code /> : <CodeOff />}
                              <Typography variant="body2" fontSize={12} textColor={475467}>  {applicants_real.status ? applicants_real.status.charAt(0).toUpperCase() + applicants_real.status.slice(1) : ''}</Typography>
                              <PeopleAltSharp />
                              <Typography variant="body2" fontSize={12} textColor={475467}> {Array.isArray(applicants_real.applicants) ? applicants_real.applicants.length : 0}</Typography>
                          </Stack>
                        </Stack>
                        {/*Description and Requirements*/}
                        <Stack spacing={2}>
                          <Typography textAlign={"left"} fontSize={20} variant="h1" color="#101828" fontWeight={550}>
                              Description
                          </Typography>
                            <Typography variant="body-lg" color="neutral" textColor={475467}>
                                {applicants_real.description?.split("\n").map((line, index) => (
                          ...
                          <Typography textAlign={"left"} fontSize={20} variant="h1" color="#101828" fontWeight={550}>
                            ...
                            <Typography variant="body-lg" color="neutral" textColor={475467}>
                                {applicants_real.requirements?.split("\n").map((line, index) => (
                                    <React.Fragment key={index}>
                                        {"->"}{line}
                                        <br />
                                    </React.Fragment>
      ...
      <p></p>
        ...
        <Grid container justifyContent="center">
          <Stack direction="row"  spacing={1} p={1.2} bgcolor={"#F9F9FB"} sx={{ width: "fit-content", borderRadius: 8, border: "1px solid #F2F4F7" }}>
              <Button

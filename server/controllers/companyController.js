...
const companyRegister = async (req, res) => {
    try {
        ...
        const { companyName, website, companyType, country, industry, companySize, companyOverview, companyWorkCulture, companyBenefits } = req.body;
        ...
        res.status(201).json({ message: "Company created successfully", companyId: savedCompany._id });
    } catch (error) {
...
const createJobPost = async (req, res) => {
    try {
        ...
        const { title, description, requirement, preferredSkills, preferredLanguages, preferredTechnologies, experience, jobType, environment, compensation } = req.body;
        ...
        res.status(201).json({ message: "Job post created successfully", jobPostId: savedJobPost._id });
    } catch (error) {
